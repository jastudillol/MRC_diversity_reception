...
private:
  ...
  uint32_t m_equivalente; //New variable to recibe equivalent m
  uint32_t o_equivalente; //New variable to recibe equivalent omega
  uint32_t max_distance;  //New variable to recibe the size of pointer mysize
 ...
 */ 
class NewPLossModel : public PropagationLossModel
{
public:
  /**
   * \brief Get the type ID.
...
private:
   ...
   * Defined and unimplemented to avoid misuse
   */
  NewPLossModel (const NewPLossModel &);
  /**
   * \brief Copy constructor
   *
   * Defined and unimplemented to avoid misuse
   * \returns
   */
  NewPLossModel & operator = (const NewPLossModel &);
 ...
 virtual double DoCalcRxPower (double txPowerDbm,
  ...
  uint32_t n_param1; //Nueva variable para recibir al m equivalente
