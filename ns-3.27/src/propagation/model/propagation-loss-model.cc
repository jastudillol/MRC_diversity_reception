 ...
 *
 * 
 * This file modifies NakagamiPropagationLossModel to add diversity in
 * reception under two equivalent parameters ("omega" and "m"). 
 * These parameters are generated by Matlab Software. In this file we  
 * prepare the source code to receive these parameters in a pointer. 
 * It's necessary to add new attributes in the Nakagami model.
 */
...
//Attribute of integer type to set the maximum distance
    .AddAttribute("maxdistance", "Dist. max,long vector",
		  IntegerValue(1000),
		  MakeIntegerAccessor(&NakagamiPropagationLossModel::max_distance),
		  MakeIntegerChecker<uint32_t>())

//Attribute of integer type to set the equivalent "m"
    .AddAttribute("mequi", "el nuevo atributo.",
		  IntegerValue(1),
		  MakeIntegerAccessor(&NakagamiPropagationLossModel::m_equivalente),
		  MakeIntegerChecker<uint32_t>())

//Attribute of integer type to set the equivalent "omega"
    .AddAttribute("omegaequi", "El nuevo atributo.",
		  IntegerValue(1),
		  MakeIntegerAccessor(&NakagamiPropagationLossModel::o_equivalente),
		  MakeIntegerChecker<uint32_t>())
...
{
   //New Pointer added
   double *m_equiv=(double*) m_equivalente;
   
   //New Pointer added
   double *o_equiv=(double*) o_equivalente;
 ...
 int index=distance;
  ...
  // Adding equivalent omega
  double powerW = o_equiv[index];
  ...
  if (int_m == m)
  ...
  else
    {
      ...
      //std::cout << "valor gama " << m_gammaRandomVariable << " \n";
    }
...
// ------------------------------------------------------------------------- //
//NEW PROPAGATION LOSS MODEL

NS_OBJECT_ENSURE_REGISTERED (NewPLossModel);

TypeId 
NewPLossModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NewPLossModel")
    .SetParent<PropagationLossModel> ()
    .SetGroupName ("Propagation")
    .AddConstructor<NewPLossModel> ()
    

    .AddAttribute("p_int", "atributo entero.",
		  IntegerValue(1),
		  MakeIntegerAccessor(&NewPLossModel::n_param1),
		  MakeIntegerChecker<uint32_t>())
  ;
  return tid;
}
NewPLossModel::NewPLossModel ()
  : PropagationLossModel ()
{
}
...
double
NewPLossModel::DoCalcRxPower (double txPowerDbm,
                                  Ptr<MobilityModel> a,
                                  Ptr<MobilityModel> b) const
{
  return 0;
...
int64_t
NewPLossModel::DoAssignStreams (int64_t stream)
{
  return 0;
}
